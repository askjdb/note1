...
我们也可以对数据库的数据进行修改，以下实例将 name 为 "菜鸟教程" 的 url 改为 https://www.runoob.com：Node.js 连接 MongoDB
...
```js
var MongoClient = require('mongodb').MongoClient; var url = "mongodb://localhost:27017/runoob";  MongoClient.connect(url, function(err, db) {  
    if (err) throw err;  
    console.log("数据库已创建!");  
    db.close(); 
});
...
如果要指定跳过的条数，可以使用 **skip()** 方法。该方法只接受一个参数，指定了跳过的条数。
