...
看用户的输入是url还是搜索内容（看是否符合url规则），url进行DNS域名解析，要是搜索内容结合搜索引擎转化为url，然后进行DNS域名解析

### DNS域名解析

在网络世界，你肯定记得住网站的名称，但是很难记住网站的 IP 地址，因而也需要一个地址簿，就是 DNS 服务器。DNS 服务器是高可用、高并发和分布式的，它是树状结构，如图：

![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e79967085e3347afb0397bdcfd97bde3~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fb6e391559114715902f2f11fe35226b~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)

递归迭代结合起来的过程，可以用一个图表示：

![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7a05842f30db483ca304fe9228380530~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
> 注：TCP三次握手完成后，若是https这时会进行SSL握手

**`备注`**

ACK：此标志表示应答域有效，就是说前面所说的TCP应答号将会包含在TCP数据包中；有两个取值：0和1，为1的时候表示应答域有效，反之为0。TCP协议规定，只有ACK=1时有效，也规定连接建立后所有发送的报文的ACK必须为1。

SYN(SYNchronization)：在连接建立时用来同步序号。当SYN=1而ACK=0时，表明这是一个连接请求报文。对方若同意建立连接，则应在响应报文中使SYN=1和ACK=1. 因此, SYN置1就表示这是一个**连接请求或连接接受报文**。
...
![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b7d1aab2c254449e8b55f2bcb1e72d52~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e5e0ba017191460f8497f3069bea3ed3~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c57f8fe01bb54faa9fa974a8b21605ee~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/231aa977b6b14062b47b6e5b7536e6ed~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)

其中，这个过程需要注意的是`回流和重绘`[《浏览器相关原理(面试题)详细总结二》](https://juejin.cn/post/6844903969693646862#heading-1)

#### 生成分层树

页面中有很多复杂的效果，如一些复杂的 3D 变换、页面滚动，或者使用 z-indexing 做 z 轴排序等，为了更加方便地实现这些效果，渲染引擎还需要为特定的节点生成专用的图层，并生成一棵对应的图层树（LayerTree），如图：

![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f4fceeee3f5841d2b71b7a6b3f8238e9~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
并不是布局树的每个节点都包含一个图层，如果一个节点没有对应的层，那么这个节点就从属于父节点的图层。那么需要满足什么条件，渲染引擎才会为特定的节点创建新的层呢？详细的可以看我另一篇文章[《浏览器相关原理(面试题)详细总结二》](https://juejin.cn/post/6844903969693646862#heading-4)，这里就不说了～

#### 栅格化

合成线程会按照视口附近的图块来优先生成位图，实际生成位图的操作是由栅格化来执行的。所谓栅格化，是指将图块转换为位图。如图：

![GitHub](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/69aadd32e58e47e18654d27ab038287a~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
