...
```js
js
复制代码type Combinable = string | number;
```
...
```js
js
复制代码Property 'split' does not exist on type 'number'.
```
...
```js
js
复制代码Tuple type '[number, string]' of length '2' has no element at index '2'.
```
...
```js
js
复制代码const point: readonly [number, number] = [10, 20];
```
...
```js
js
复制代码const lolo: Sem = { name: "lolo", age: 5 }
```
...
```vbnet
vbnet复制代码let K1: keyof boolean; // let K1: "valueOf"
...
```js
js
复制代码Element implicitly has an 'any' type because expression of type 'string' can't be used to index type '{}'.
```
...
```js
js
复制代码Argument of type '"date"' is not assignable to parameter of type '"id" | "text" | "done"'.
```
...
```js
js
复制代码loggingIdentity(3);  // Error, number doesn't have a .length property
```
...
```js
js
复制代码loggingIdentity({length: 10, value: 3});
```
...
```js
js
复制代码type Exclude<T, U> = T extends U ? never : T;
```
...
```js
js
复制代码type Extract<T, U> = T extends U ? T : never;
```
...
```js
js
复制代码type Omit<T, K extends keyof any> = Pick<T, Exclude<keyof T, K>>;
```
...
```js
js
复制代码type NonNullable<T> = T extendsnull | undefined ? never : T;
```
...
[Node.js模块里exports与module.exports的区别?](https://link.juejin.cn?target=https%3A%2F%2Fwww.zhihu.com%2Fquestion%2F26621212)
...
```js
js
复制代码type PersonWithBirthDate = Person & { birth: Date };
```
...
```js
js
复制代码type Options = typeof INIT_OPTIONS;
```
